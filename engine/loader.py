...
def _propagate_config_fields(config, cfg: GlobalConfig):
    ...
    config.linear_attn_prefill_backend = cfg.model.linear_attn_prefill_backend
    config.linear_attn_decode_backend  = cfg.model.linear_attn_decode_backend
    config.kv_cache_max_len            = cfg.model.kv_cache.max_len
    config.preallocate_cache           = cfg.model.use_cuda_graph
...
def _load_weights(model, cfg: GlobalConfig, data_path: str):
    ...
    if cfg.model.use_quantized_model:
        ...
        offload_paths = list(cfg.model.cpu_offload_modules)
...
def load_model(cfg: GlobalConfig):
    ...
    if cfg.model.use_quantized_model and backend == "megakernel_cuda":
        raise RuntimeError(
    ...
    config_path = (
        data_path if cfg.model.use_quantized_model else cfg.path.model_path
    )
    ...
    if backend == "megakernel_cuda":
        ...
        model = create_megakernel_model(model, variant=cfg.model.megakernel_variant)
