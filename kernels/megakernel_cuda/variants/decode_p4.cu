/**
 * decode_p4.cu — naive + P4 (vectorized attention + uint4 weight loads).
 *
 * Forked from ``decode_naive.cu``. The only changes vs. naive are the P4
 * micro-optimizations described in upstream's Round 5:
 *
 *   1. Attention (``naive_attention``):
 *        - Cache the per-lane Q slice in registers (``q_local[4]``) once
 *          before the position loop, instead of re-reading
 *          ``q_head[q_idx + N]`` each iteration.
 *        - Load 4 bf16 K values per lane with a single ``uint2 __ldg``
 *          instead of four scalar ``__ldg`` of bf16. Same for V.
 *
 *   2. QKV / O / gate / up / down matvecs:
 *        - Replace the scalar ``__ldg<__nv_bfloat16>`` weight stream
 *          (stride ``WARP_SIZE``) with a ``uint4 __ldg`` (8 bf16 weights /
 *          lane, stride ``WARP_SIZE * 8``).
 *
 *   3. ``naive_o_proj_postnorm_mlp``:
 *        - Stage ``attn_out`` into ``__shared__ float s_attn[Q_SIZE]`` once
 *          at the start, so the O-proj matvec reads from SMEM instead of
 *          global.
 *        - Stage ``g_mlp_intermediate`` into ``__shared__ float
 *          s_mlp[INTERMEDIATE_SIZE]`` after the gate/up sync, so the
 *          down-proj matvec reads from SMEM.
 *
 * Everything else is identical to naive:
 *   - Cooperative-groups ``cg::this_grid().sync()`` at every layer boundary
 *     (P3 still reverted).
 *   - RMSNorm computed by block-0 only, then broadcast through global
 *     ``g_normalized`` with a grid.sync (P1 still reverted). Hence the QKV
 *     and gate/up matvecs still read ``g_normalized`` from global, not SMEM.
 *   - No L2 prefetch (P2 / P5 / P6 / P7 / P9 / P10 still reverted).
 *   - No SMEM stride-9 padding (P8 still reverted, ``s_attn`` / ``s_mlp``
 *     use plain ``[N]`` layout).
 *
 * ABI: ``extern "C" launch_ldg_decode[_with_logits]`` signature and the
 * ``LDGLayerWeights`` struct are unchanged so ``decode_wrapper.cpp`` works
 * for this variant without modification.
 */
    ...
    float *__restrict__ v_out)
{
    ...
    for (int m_base = row_start; m_base < row_end; m_base += LDG_NUM_WARPS)
    {
        ...
        // P4: uint4 weight load (8 bf16 weights / lane); g_normalized still
        // read from global (P1 reverted, no SMEM cache).
        float sum = 0.0f;
#pragma unroll 4
        for (int k = lane_id * 8; k < HIDDEN_SIZE; k += WARP_SIZE * 8)
        {
            ...
            sum += __bfloat162float(w_ptr[0]) * g_normalized[k + 0] +
    ...
    float attn_scale)
{
    ...
    if (block_id < ATTN_BLOCKS)
    {
        ...
        // P4: cache Q head in registers using contiguous vec4 pattern
        // (lane N reads 4N..4N+3). Loaded once before the position loop.
        int q_idx = lane_id * 4;
        float q_local[4];
        ...
        for (int pos = warp_id; pos < cache_len; pos += LDG_NUM_WARPS)
        {
            ...
            // P4: vectorized uint2 load for K cache (4 bf16 / lane in 1 txn).
            float score = 0.0f;
            ...
            // P4: vectorized uint2 load for V cache.
    ...
    __nv_bfloat16 *__restrict__ hidden_out)
{
    ...
    // P4: stage attn_out into shared memory so the O-proj matvec below reads
    // the same Q_SIZE-wide vector repeatedly out of SMEM instead of global.
    ...
    for (int m_base = hid_start; m_base < hid_end; m_base += LDG_NUM_WARPS)
    {
        ...
        // P4: uint4 weight load (8 bf16 / lane); attn_out read from s_attn.
        float sum = 0.0f;
#pragma unroll 4
        for (int k = lane_id * 8; k < Q_SIZE; k += WARP_SIZE * 8)
        {
            ...
            __nv_bfloat16 *w_ptr = reinterpret_cast<__nv_bfloat16 *>(&w_u4);
            sum += __bfloat162float(w_ptr[0]) * s_attn[k + 0] +
                   __bfloat162float(w_ptr[1]) * s_attn[k + 1] +
                   __bfloat162float(w_ptr[2]) * s_attn[k + 2] +
                   __bfloat162float(w_ptr[3]) * s_attn[k + 3] +
                   __bfloat162float(w_ptr[4]) * s_attn[k + 4] +
                   __bfloat162float(w_ptr[5]) * s_attn[k + 5] +
                   __bfloat162float(w_ptr[6]) * s_attn[k + 6] +
                   __bfloat162float(w_ptr[7]) * s_attn[k + 7];
        }
        sum = warp_reduce_sum(sum);
        if (lane_id == 0)
    ...
    // Gate + Up + SiLU
    {
        ...
        for (int m_base = int_start; m_base < int_end; m_base += LDG_NUM_WARPS)
        {
            ...
            // P4: uint4 gate/up weight loads (8 bf16 / lane). g_normalized
            // still global because the post-attn RMSNorm broadcast is naive
            // style (P1 reverted, single-block writer + grid.sync).
...
#pragma unroll 4
            for (int k = lane_id * 8; k < HIDDEN_SIZE; k += WARP_SIZE * 8)
            {
                ...
                float s0 = g_normalized[k + 0], s1 = g_normalized[k + 1],
                      s2 = g_normalized[k + 2], s3 = g_normalized[k + 3],
                      s4 = g_normalized[k + 4], s5 = g_normalized[k + 5],
                      s6 = g_normalized[k + 6], s7 = g_normalized[k + 7];
    ...
    // P4: stage g_mlp_intermediate into shared memory so the down-proj
    // matvec below reads the same INTERMEDIATE_SIZE vector repeatedly out of
    // SMEM instead of global. The grid.sync above guarantees every block
    // can see every other block's writes to g_mlp_intermediate.
    ...
    for (int m_base = hid_start; m_base < hid_end; m_base += LDG_NUM_WARPS)
    {
        ...
        // P4: uint4 weight load (8 bf16 / lane); mlp_intermediate from s_mlp.
        float sum = 0.0f;
#pragma unroll 4
        for (int k = lane_id * 8; k < INTERMEDIATE_SIZE; k += WARP_SIZE * 8)
        {
            ...
            sum += __bfloat162float(d_ptr[0]) * s_mlp[k + 0] +
                   __bfloat162float(d_ptr[1]) * s_mlp[k + 1] +
                   __bfloat162float(d_ptr[2]) * s_mlp[k + 2] +
                   __bfloat162float(d_ptr[3]) * s_mlp[k + 3] +
                   __bfloat162float(d_ptr[4]) * s_mlp[k + 4] +
                   __bfloat162float(d_ptr[5]) * s_mlp[k + 5] +
                   __bfloat162float(d_ptr[6]) * s_mlp[k + 6] +
                   __bfloat162float(d_ptr[7]) * s_mlp[k + 7];
        }
        sum = warp_reduce_sum(sum);
        if (lane_id == 0)
