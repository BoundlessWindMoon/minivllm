/**
 * decode_p3.cu — naive + P3 (AtomicGridSync + flag-based partial barriers).
 *
 * Forked from ``decode_naive.cu``. Only the synchronization mechanism is
 * touched; every device function body (RMSNorm, QKV, QK-norm/RoPE/cache,
 * attention, O+post-norm+MLP) is byte-identical to the naive baseline. This
 * isolates the P3 contribution.
 *
 * P3 changes vs naive:
 *   - Replace cooperative-groups ``cg::this_grid().sync()`` with a hand-rolled
 *     ``AtomicGridSync`` struct (atomic counter + spin-wait on a generation
 *     counter). Kernel launch becomes a normal ``<<<>>>`` instead of
 *     ``cudaLaunchCooperativeKernel``.
 *   - Two of the eight per-layer grid syncs become **partial barriers**:
 *       * ``kv_flag``  — only the 16 attention blocks (block_id < ATTN_BLOCKS)
 *         atomic-increment + spin on this counter after QK-norm/RoPE/cache.
 *         The remaining 4 non-attention blocks return immediately and skip
 *         this barrier (they have nothing to wait for).
 *       * ``attn_flag`` — attention blocks signal completion via this counter;
 *         non-attention blocks wait on it to gate the post-attention work.
 *   - ``__launch_bounds__(LDG_BLOCK_SIZE, 1)`` keeps one block per SM so the
 *     spin-wait cannot deadlock from co-resident blocks.
 *
 * P3 keeps these naive behaviours (so the delta is purely sync mechanism):
 *   - Block-0-only RMSNorm with ``g_normalized`` broadcast (P1 reverted).
 *   - Scalar bf16 weight / KV-cache loads (P4 / P6 reverted).
 *   - No L2 prefetch (P2 / P5 / P6 / P7 / P9 / P10 reverted).
