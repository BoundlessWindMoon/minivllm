/**
 * decode_p0.cu — Placeholder for P0 "L2 cache thrashing fix on layers 10-12
 * qkv_projection" round.
 *
 * Upstream optimization_log.md (referenced by the ablation plan) is not
 * present in this repository, and decode_ldg.cu carries no comment marking
 * the specific code change introduced in P0. With no concrete diff to
 * reproduce, this variant is **byte-identical to decode_naive.cu**.
 *
 * Purpose:
 *   - Reserve the `p0` slot in `MINI_VLLM_MK_VARIANT` so the overview table
 *     has an entry for the round.
 *   - Provide a verify/bench/NCU run that matches naive exactly, confirming
 *     that no "layer 10-12 qkv anomaly" pops up on 4050 (i.e. P0 has nothing
 *     to fix on this hardware, or the fix is not what the upstream label
 *     suggests).
 *
 * See docs/megakernel_ablation/02_p0_l2_thrash.md for the writeup.
 */
/**
 * decode_naive.cu — Megakernel baseline.
 *
 * This variant strips out every per-round optimization (P0..P10) from
 * ``decode_ldg.cu`` and only keeps the megakernel idea itself:
 *   single persistent kernel that runs the full Qwen3-0.6B decode pass
 *   (embed -> N x layer -> final norm) before launching the LM head.
 *
 * Choices that distinguish ``naive`` from the optimized version:
 *   - Cooperative-groups grid sync (``cg::this_grid().sync()``) everywhere.
 *     No AtomicGridSync, no partial barriers (P3 reverted).
 *   - RMSNorm is done by block 0 only into global memory, followed by a
 *     grid.sync so every block sees the result (P1 reverted).
 *   - Weight reads use scalar ``__ldg<__nv_bfloat16>`` (P4 / P6 reverted —
 *     no uint4 / vec4 weight loads).
 *   - KV cache reads are scalar bf16 (P4 reverted — no uint2 vec loads).
 *   - Attention reads Q from global memory; no register cache (P4 reverted).
 *   - ``attn_out`` and ``mlp_intermediate`` are read directly from global in
 *     downstream projections (P4 / P8 reverted — no SMEM cache, no padding).
 *   - No L2 prefetch helpers anywhere (P2 / P5 / P6 / P7 / P9 / P10 reverted).
 *   - SMEM indexing uses ``SMEM_PAD_IDX(i) == i`` (P8 reverted).
 *
