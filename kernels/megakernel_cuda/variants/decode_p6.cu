/**
 * decode_p6.cu — naive + P3 (atomic sync) + P6 (idle-block L2 prefetch).
 *
 * Forked from ``decode_p3.cu``. Builds on P3's partial-barrier infrastructure
 * (since the prefetch only makes sense when the 4 non-attention blocks can
 * skip the attention work, which P3 enables via ``attn_flag``).
 *
 * P6 changes vs P3:
 *   - New device helper ``ldg_prefetch_weights_l2`` (``__ldg``-style streaming
 *     loads at 128-byte cache-line stride). The body issues
 *     ``ld.global.nc.v4.b32`` (== ``__ldg<uint4>``) loads, one per cache line
 *     per thread per outer iter, and discards the value via a volatile asm
 *     sink. Stride = LDG_BLOCK_SIZE × 128 bytes per iteration so the 256
 *     threads collectively touch 256 cache lines per pass — exactly P6's
 *     "full cache line coverage" fix (P5 only covered partial lines).
 *   - In ``naive_attention``, the idle-block branch (block_id >= ATTN_BLOCKS)
 *     no longer just spin-waits on ``attn_flag``. It first splits the 4 idle
 *     blocks into 4 prefetch groups (O proj / gate proj / up proj / down
 *     proj), each block prefetching its slice of the current layer's weights
 *     into L2. Then it joins the ``attn_flag`` spin like P3 did.
 *   - ``naive_attention``'s signature now takes ``o_weight``, ``gate_weight``,
 *     ``up_weight``, ``down_weight`` so the idle blocks know what to prefetch.
 *
 * What is NOT in P6 (these are later rounds):
 *   - PTX ``prefetch.global.L2::evict_last`` — that's P7's swap-in.
 *   - Fire-and-forget DRAM prefetch at the 4 sync windows — that's P9.
 *   - Site-#4 next-layer-QKV prefetch relocation — that's P10.
 *
 * P6 keeps these naive / P3 behaviours (so the delta is purely the prefetch):
...
// =============================================================================
// P6 idle-block L2 prefetch helper (using __ldg-style streaming loads).
// =============================================================================
//
// Each thread issues one ``ld.global.nc.v4.b32`` (== __ldg<uint4>, 16 bytes)
// per outer iteration; the value is sunk into a volatile asm so the compiler
// cannot DCE the load. The thread stride is 128 bytes (one full L2 cache
// line on sm_89), so 256 threads × 128 bytes = 32 KB per iter = 256 cache
// lines per iter. This is the P6 "full cache line coverage" fix — every
// cache line in the prefetch range gets exactly one touch, no gaps, no
// duplicates. P7 swaps this for ``prefetch.global.L2::evict_last``.
    ...
    const __nv_bfloat16 *__restrict__ weights, int num_elements)
{
         ...
         offset += LDG_BLOCK_SIZE * 128)
    {
        ...
        asm volatile("ld.global.nc.v4.b32 {%0,%1,%2,%3}, [%4];"
                     : "=r"(dummy.x), "=r"(dummy.y), "=r"(dummy.z), "=r"(dummy.w)
                     : "l"(base + offset));
        // Force the compiler to keep the load (otherwise DCE would erase it).
