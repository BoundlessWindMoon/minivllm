    ...
    __nv_bfloat16 *__restrict__ hidden_out)
{
    ...
    // P9 site #4 (post-GU placement): prefetch next layer's Q/K/V weights
    // (8 MB combined) while we wait for the post-gate/up grid.sync. P10 will
    // move this same prefetch to after down proj. Skipped on the last layer.
    if (!is_last_layer)
    {
